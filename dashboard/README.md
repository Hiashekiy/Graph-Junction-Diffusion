...
播放条上的 **视野** 选择器有两个取值：**自适应**（默认）/ **全图**。

**为什么需要**：真实 corridor 有 900~2900 个节点（长集平均 1900），而一条路线只走过
其中几十个 —— 全图铺满画布时，路线只占几个像素、decision 圆点糊成一片白点，
样本越密越没法看。

**做法**：把 SVG 的 `viewBox` 收紧到「**显示出来的路线 + GT**」的包围盒上（留 10%
边距、保持 1000:620 长宽比、最多放大 6 倍）。收紧之后 SVG 会**按比例放大一切** ——
节点圆点、路线线宽、并行车道间距一起变大，这正是想要的效果。只有两样要单独交代：

| 元素 | 处理 | 原因 |
|---|---|---|
| 文字（节点标签 / 比例尺） | 字号与偏移按 **1/scale** 缩回 | CSS 规则优先于 SVG 表现属性，所以写 inline style；不缩回的话放大 6 倍后标签有 78px 高、且飘到节点上方 78px |
| 街道底图 `.street-edge` | 不变（本来就是 `vector-effect: non-scaling-stroke`） | 放大后仍是细线，不会把画面压死 |

比例尺也跟着**可视区**走（贴在左下角、按屏幕像素挑"好看"的整数公里数），
否则一收紧 viewBox 它就跑到画面外了。扩散过程页签会**复位** viewBox ——
它有自己的 caption 坐标。

**配套的 节点 开关**（默认 **仅路线**）：真实 corridor 有上千个 decision 节点，
全画成小圆会糊成一片白点、把路线和路网底图都盖住 —— 只画「显示出来的路线经过的
节点」+ S/G 就够读了。想回到老画法选 **全部**，**全关** 则一个圆点都不画（只留线）。
两个开关（视野 / 节点）改动后都**原位重画**，不会从头重播。

`全图` 模式恢复整幅画布，与改动前逐位一致。改视野**不会从头重播**：
先记下当前播放进度，重画完原位填回去。

...
用法::
    ...
    python tools/evaluate_multipath.py --run outputs/runs/v2_rev2_mixed \
        --data data/controlled_long.pkl --top-k 2 --beam-width 64
...
def evaluate_seed(
    ...
    for batch_samples in iter_batches(list(samples), batch_size=args.batch_size, shuffle=False):
        ...
        for index, sample in enumerate(batch_samples):
            single = decode_flat(
                sample, z0,
                decision_offset=d_offsets[index],
                candidate_offset=c_offsets[index],
            )
        ...
        if args.show:
            ...
            for row in result.get("details", []):
                print(f"    #{row['index']:<5d} dec={row['num_decisions']:<3d} gt={row['gt_length']:<3d} "
                      f"单路径={row['single']:<7s} best={row['best']:<7s} cost={row['best_cost']} "
                      f"coverage={row['coverage']} goal_paths={row['goal_paths']}")
        print()

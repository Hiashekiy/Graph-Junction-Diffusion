...
用法::
    ...
    # 单条：看第 170 号 query
    python tools/predict_path.py --run outputs/runs/controlled_unweighted \
        --data data/unweighted/unweighted_test.pkl --index 170

    # 多条（逗号分隔或重复 --index），并导出 JSON
    python tools/predict_path.py --run outputs/runs/controlled_unweighted \
        --data data/unweighted/unweighted_test.pkl --index 0,47,170 --out-json paths.json

    # 推理轮数 ablation：同一个 checkpoint 只跑 1 轮
    python tools/predict_path.py --run outputs/runs/controlled_unweighted \
        --data data/unweighted/unweighted_test.pkl --index 170 --flow-steps 1

    # 换随机种子（采样是随机的，同一 query 换个种子可能走出不同路径）
    python tools/predict_path.py --run outputs/runs/controlled_unweighted \
        --data data/unweighted/unweighted_test.pkl --index 170 --seed 3
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data", required=True, help="数据集 pkl，如 data/unweighted/unweighted_test.pkl")
    parser.add_argument(
